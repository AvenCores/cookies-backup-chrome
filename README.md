...
Про `.crx`: файл подписан эфемерным ключом CI (валидный формат CRX3 с proof-подписью — проверено разбором заголовка: `Cr24`, version 3, `sha256_with_rsa` + `signed_header_data` на месте), но Chrome/Edge/Brave Stable такой файл ставить откажутся с ошибкой `Пакет недействителен: "CRX_REQUIRED_PROOF_MISSING"` — для перетаскивания на `chrome://extensions` Google требует подпись самого Web Store, самоподпись автора недостаточна. Это ограничение браузеров, а не баг сборки. Варианты:
- основной путь — `.zip` как «распакованное расширение» (см. выше);
- `.crx` — это обычный zip-архив: его можно переименовать в `.zip`, распаковать и тоже загрузить распакованным;
- напрямую `.crx` ставится в Chromium-форки без проверки подписи (Ungoogled Chromium, Kiwi на Android и т.п.).
Чтобы зафиксировать стабильный extension ID `.crx`, сгенерируйте ключ один раз (`npx --yes crx@5 keygen ./crx-key`) и положите его содержимое в секрет репозитория `CRX_PRIVATE_KEY` (Settings → Secrets → Actions, raw PEM или base64) — CI подхватит его автоматически.
